...
import { SiWordpress, SiNextdotjs, SiReact, SiTailwindcss, SiSass, SiGoogleanalytics, SiGoogletagmanager, SiAdobeillustrator, SiAdobephotoshop, SiFirebase, SiGooglebigquery } from "react-icons/si";
...
export default function About() {
  ...
  // Different parallax rates for each column with spring easing
  const leftColumnY = useSpring(useTransform(scrollYProgress, [0, 1], [100, -100]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
  const rightColumnY = useSpring(useTransform(scrollYProgress, [0, 1], [150, -150]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });

  // Heading split parallax - slides from opposite sides to meet in middle with easing
  const leftHeadingX = useSpring(useTransform(scrollYProgress, [0, 0.3], [-200, 0]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
  const rightHeadingX = useSpring(useTransform(scrollYProgress, [0, 0.3], [200, 0]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
    ...
    <section ref={sectionRef} className="bg-purple-50 relative">
      ...
      <div className="mx-auto px-4 py-24 pt-32">
        ...
        <div className="flex flex-col md:flex-row items-start justify-center gap-6">
          {/* Left text block */}
          <motion.div className="w-full h-full md:w-auto text-center relative top-24" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={blurInUpVariant} transition={{ delay: 0.4 }}>
            <motion.div style={{ y: leftColumnY }} className="motion-col-parallax">
              <GradientText colors={["#3c0366", "#8660faff", "#a855f7ff", "#8400ffff", "#a855f7ff", "#3c0366"]} animationSpeed={15} showBorder={false} className="text-8xl !font-bold p-1">
                Alfredo
              </GradientText>
              ...
              <div className="flex items-center  justify-between gap-3 mt-4 flex-wrap">
                ...
                <span className="inline-block transition-transform duration-200 ease-out hover:scale-110 cursor-pointer">
                  <SiGoogleanalytics className="text-[#e37400]" size={40} title="Google Analytics 4" />
                </span>
                <span className="inline-block transition-transform duration-200 ease-out hover:scale-110 cursor-pointer">
                  <SiGoogletagmanager className="text-[#246fdb]" size={40} title="Google Tag Manager" />
                </span>
              </div>
          ...
          {/* Right text block */}
          <motion.div className="w-full h-full md:w-auto text-center flex flex-col gap-1 align-center relative top-[14rem]" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={blurInUpVariant} transition={{ delay: 0.4 }}>
            <motion.div style={{ y: rightColumnY }} className="motion-col-parallax">
              ...
              <div className="flex items-center justify-between gap-3 mt-4 flex-wrap">
                <span className="inline-block transition-transform duration-200 ease-out hover:scale-110 cursor-pointer">
                  <SiAdobeillustrator className="text-[#ff9a00]" size={40} title="Adobe Illustrator" />
                </span>
                <span className="inline-block transition-transform duration-200 ease-out hover:scale-110 cursor-pointer">
