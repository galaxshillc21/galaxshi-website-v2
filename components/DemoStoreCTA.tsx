...
type DemoStoreCTAProps = {
  content: SiteDictionary["demoCta"];
};

export default function DemoStoreCTA({ content }: DemoStoreCTAProps) {
  return (
        ...
        <div className="rounded-2xl border border-cyan-200 bg-gradient-to-r from-cyan-50 to-sky-100 p-8 shadow-sm border-[2px] transition-all duration-300 hover:border-cyan-400 hover:shadow-lg">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="max-w-2xl">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-700">{content.eyebrow}</p>
              <h2 className="mt-2 text-2xl font-bold text-slate-900">{content.title}</h2>
              <p className="mt-2 text-slate-700">{content.description}</p>
            </div>
